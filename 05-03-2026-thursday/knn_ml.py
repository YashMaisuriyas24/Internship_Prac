...
class  KNearestNeighbours:
    ...
    def save_model(self, k=5, filename="knn_models.pkl"):
        model = {
        ...
        for name, params in model.items():
            ...
            print(f"{name.capitalize()} trained. Accuracy: {acc:.2%}")
...
if __name__ == "__main__":
    knn = KNearestNeighbours(file_path="pulsar_stars.csv",target_column="target_class")
    knn.load_data()
    knn.perform_eda()
    knn.remove_outliers()
    knn.split_data()
    knn.grid_search_svm()
    knn.evaluate_model()
    knn.save_model(filename="knn_models.pkl")
